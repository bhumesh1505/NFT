    ...
    def loadModel(self, detection_speed="normal"):
        ...
        if(self.__modelType == "retinanet"):
            if (detection_speed == "normal"):
                self.__input_image_min = 600
                self.__input_image_max = 600
        ...
        if(self.__modelLoaded == False):
            ...
            try:
                if (self.__modelType == "retinanet"):
                    ...
                    setBits = np.zeros(shape=(self.__input_image_min, self.__input_image_max)).astype(int)
                    ...
                    for index, (label, score), in enumerate(zip(predicted_numbers, scores)):
                        ...
                        # detection_c = detections[0, index, :4]           ## CHANGES find coordinates wrt resized img
                        # detection_c *= scale
                        # detection_c = detection_c.astype(int)
                        x1 , y1 , x2, y2 = detection_details
                        # print(x1,end=" ")
                        # print(y1)
                        # print(x2,end=" ")
                        # print(y2)
                    ...
                    # for i in range(0,self.__input_image_min):               ## printing bit array
                    #     for j in range(0,self.__input_image_max):
                    #         print(setBits[i][j],end="")
                    #     print()

                    ## setBits[i][j] means bit at row i and column j


                    stepSize = 30

                    reducedBits = np.zeros(shape = (19,19)).astype(int)         ## 0,30,60,90,....540 can be starting coordinate of window, so (600/30) - 1
                    rows = 0
                    cols = 0

                    for y in range(0, self.__input_image_min - 60, stepSize):           ## rows 
                        for x in range(0, self.__input_image_max - 60, stepSize):       ## columns
                            # yield the current window
                            found = 0
                            for i in range(y,y+60):
                                for j in range(x,x+60):
                                    if setBits[i][j] > 0:
                                        reducedBits[rows][cols] = 1
                                        found = 1
                                        break
                                if found == 1:
                                    break
                    ...
                    if (extract_detected_objects == True):
                        ...
                        if (output_type == "file"):
                            return output_objects_array, b				# return array as well
                        elif (output_type == "array"):
